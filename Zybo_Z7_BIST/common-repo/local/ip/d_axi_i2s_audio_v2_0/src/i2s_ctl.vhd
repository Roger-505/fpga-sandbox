...
--      CLK_100MHZ_I : in  std_logic; -- System clock (100 MHz)
      ...
      EN_RX_I     : in  std_logic; -- Receive enable
		...
		D_L_I       : in  std_logic_vector(31 downto 0); -- Left channel data
      D_R_I       : in  std_logic_vector(31 downto 0); -- Right channel data
      ...
      D_L_O       : out std_logic_vector(31 downto 0); -- Left channel data
      D_R_O       : out std_logic_vector(31 downto 0); -- Right channel data
...
--
signal D_L_O_int              : std_logic_vector(31 downto 0);

--
signal D_R_O_int              : std_logic_vector(31 downto 0);

--
--signal EN_TX_I_int            : std_logic;
--signal EN_RX_I_int            : std_logic;
...
signal DBG_CNT_LRCLK               : integer range 0 to 31;

attribute KEEP : string; 
attribute KEEP of DBG_CNT_LRCLK     : signal is "TRUE";
...
------------------------------------------------------------------------
   ...
   begin
      if rising_edge(CLK_I) then
         if RST_I = '1' then
            Data_Out_int(31 downto 0) <= D_L_I; -- Left channel data by default
         elsif Cnt_Lrclk = 0 and BCLK_Rise = '1' then -- load par. data
            if LRCLK_int = '1' then
               Data_Out_int(31 downto 0) <= D_R_I;
            else
               Data_Out_int(31 downto 0) <= D_L_I;
            end if;
   ...
   begin
      if rising_edge(CLK_I) then
         if RST_I = '1' then
            ...
            if LRCLK_int = '1' then
               D_L_O_int <= Data_In_int(31 downto 0);
               Data_In_int <= (others => '0');
            else
               D_R_O_int <= Data_In_int(31 downto 0);
               Data_In_int <= (others => '0');
            end if;

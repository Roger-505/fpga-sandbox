...
void DI2s_Reset(DI2s * InstancePtr)
{
	...
	DI2s_WriteReg(InstancePtr->BaseAddress, DI2S_FIFO_CTL_OFFSET, DI2s_BitField.l);
	...
	DI2s_BitField.bit.u32bit31 = 0; // clear DI2S_RX_FIFO_RST bit
	DI2s_WriteReg(InstancePtr->BaseAddress, DI2S_FIFO_CTL_OFFSET, DI2s_BitField.l);
}
...
/****************************************************************************/
/**
* This function will start playing and immediately recording the number of
* specified samples. It is a simple acquisition mode and will not utilize
* either the DMA or the streaming capabilities of the core
*
* @param	InstancePtr is a pointer to an DI2s instance. The memory the
*		pointer references must be pre-allocated by the caller. Further
*		calls to manipulate the driver through the DI2s API must be
...
* @param	NrSamples Number of samples to be recorded
*
* @param	PalybackAddr specifies the Address in the memory where the
* 		stored samples are located.
*
* @param	RecordAddr specifies the Address in the memory where the
* 		acquired samples will be store.
*
* @return
* 			None.
...
*
*****************************************************************************/
void DI2s_LoopBack(DI2s *InstancePtr, u32 u32NrSamples,
			u32 *PalybackAddr, u32* RecordAddr)
{
	u32 i = 0;
	...
	while (i < u32NrSamples)
	{
		...
		{
			//store the samples
			DI2s_BitField.l = *(PalybackAddr + i);
...
{
	u32 Status;
	DI2s_StartI2s(InstancePtr, DI2S_TRANSMIT | DI2S_RECEIVE);
...
void DI2s_DmaLoopBack(DI2s *InstancePtr,XAxiDma *DMAInstPtr,
		u32 u32NrSamples, u32 SourceAddr, u32 DestAddr)
{
	u32 Status;

	//Set the direction and start the transactions
	DI2s_StopI2s(InstancePtr, DI2S_RECEIVE | DI2S_TRANSMIT);

	//Reset I2S core
	DI2s_Reset(InstancePtr);
    DI2s_SetNrSamples(InstancePtr, u32NrSamples);

	Status = XAxiDma_SimpleTransfer(DMAInstPtr, SourceAddr, 4*u32NrSamples, XAXIDMA_DMA_TO_DEVICE);
	if (Status != XST_SUCCESS)
	{
		xil_printf("\n fail @ play; ERROR: %d", Status);
	}

	Status = XAxiDma_SimpleTransfer(DMAInstPtr, DestAddr, 4*u32NrSamples, XAXIDMA_DEVICE_TO_DMA);
	if (Status != XST_SUCCESS)
	{
		xil_printf("\n fail @ rec; ERROR: %d", Status);
	}
	...
	//Set the direction and start the transactions
	DI2s_StartI2sStream(InstancePtr, DI2S_RECEIVE | DI2S_TRANSMIT);

	//Set the direction and start the transactions
	DI2s_StartI2s(InstancePtr, DI2S_RECEIVE | DI2S_TRANSMIT);
